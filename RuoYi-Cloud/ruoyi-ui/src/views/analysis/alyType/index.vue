<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="名称" align="center" prop="projectTypeName"/>
      <el-table-column label="代码" align="center" prop="projectTypeCode"/>
    ...
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="名称" prop="projectTypeName">
          <el-input v-model="form.projectTypeName" placeholder="请输入名称"/>
        </el-form-item>
        <el-form-item label="代码" prop="projectTypeCode">
          <el-input v-model="form.projectTypeCode" placeholder="请输入代码"/>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.loading = true
        listAlyType(this.queryParams).then(response => {
          this.alyTypeList = response.rows
      ...
      handleAdd() {
        this.reset()
        this.open = true
        this.title = '添加 分析项目类型'
      },
      /** 修改按钮操作 */
      handleUpdate(row) {
        this.reset()
        const projectTypeId = row.projectTypeId || this.ids
        getAlyType(projectTypeId).then(response => {
          this.form = response.data
          this.open = true
          this.title = '修改 分析项目类型'
        })
      },
      /** 提交按钮 */
      submitForm() {
        this.$refs['form'].validate(valid => {
          if (valid) {
            if (this.form.projectTypeId != null) {
              updateAlyType(this.form).then(response => {
                this.msgSuccess('修改成功')
                this.open = false
                this.getList()
              })
            } else {
              addAlyType(this.form).then(response => {
      ...
      handleDelete(row) {
        ...
        this.$confirm('是否确认删除 分析项目类型编号为"' + projectTypeIds + '"的数据项?', '警告', {
      ...
      handleExport() {
        ...
        this.$confirm('是否确认导出所有 分析项目类型数据项?', '警告', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(function () {
